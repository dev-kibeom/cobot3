...
처음부터 hard/glare 데이터로 가면 사각형 형태 자체를 놓칠 수 있으므로, 먼저 `normal` 난이도로 쉬운 데이터를 만듭니다. 이 생성기는 기본으로 일반 조명/일반 바닥, 약한 밝기 변화, 완만한 카메라 거리 변화만 사용합니다.

주의: 다운로드한 `iron_cube.usd`, `iron_panel.usd`는 `tools/isaac/assets/`에 보관되어 있지만, 현재 파일을 Isaac stage에 직접 reference하면 앱이 조용히 종료되는 문제가 있어 기본 생성 경로에서는 안정적인 cuboid geometry를 사용합니다. 필요할 때만 `--use-cube-usd`/`--use-plate-usd`로 실험합니다.
...
/home/rokey/dev_ws/isaac_sim/isaacsim/_build/linux-x86_64/release/python.sh \
  ...
  --count 1200 \
  --difficulty normal \
  --dataset-dir work/datasets/metal_objects_normal_usd \
  --camera-z-min 1.15 \
  --camera-z-max 1.55 \
  --cube-camera-z-min 0.95 \
  --cube-camera-z-max 1.10 \
  --clean
```

거리 변화에 덜 흔들리도록 YOLO multi-scale과 약한 scale augmentation을 켠 상태로 fine-tuning합니다.

```bash
python3 tools/training/train_yolo11s_obb.py \
  --data work/datasets/metal_objects_normal_usd/metal_objects_obb.yaml \
  --name yolo11s_obb_metal_normal_usd \
  --epochs 40 \
  --scale 0.25 \
  --translate 0.03
```

학습 후 `work/runs/metal_objects_obb/yolo11s_obb_metal_normal_usd/weights/best.pt`를 launch의 `model:=...`로 지정해서 확인합니다. 모션으로 넘길 때는 detection confidence를 `0.70` 이상에서만 통과시키는 식으로 안전하게 거르는 것을 권장합니다.
...
모션컴이 `geometry_msgs/msg/PoseStamped`의 `/isaac/goal_pos_topic`을 받는 구조라면 같은 gateway에서 pick 좌표를 `PoseStamped`로도 발행합니다. orientation은 예시 명령의 `{x: 0.0, y: 1.0, z: 0.0, w: 0.0}`과 맞도록 기본 pitch 180도에 YOLO/brain에서 계산한 yaw를 더합니다.
...
ros2 run vision goal_gateway \
  ...
  --input-topic /m0609/vision/pick_place_goal \
  --output-topic /m0609_vision/pick_place_goal \
  --pose-output-topic /isaac/goal_pos_topic \
  --pose-source pick \
  --pose-frame-id map \
  --pose-pitch-deg 180 \
  --min-confidence 0.70
