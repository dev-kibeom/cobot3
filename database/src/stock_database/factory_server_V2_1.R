...
open_terminal <- function(title, cmd, geometry="120x40") {
  ...
  full_cmd <- switch(term,
    "gnome-terminal" = sprintf(
      "nohup gnome-terminal --window --geometry=%s --title='%s' -- bash -c '%s' >/dev/null 2>&1 &",
      geometry, title, cmd),
    "xterm" = sprintf(
      "nohup xterm -title '%s' -geometry %s -e bash -c '%s' >/dev/null 2>&1 &",
      title, geometry, cmd),
    "konsole" = sprintf(
      "nohup konsole -- bash -c '%s' >/dev/null 2>&1 &",
      cmd),
    sprintf(
      "nohup %s --geometry=%s --title '%s' -- bash -c '%s' >/dev/null 2>&1 &",
...
start_monitor_watchdog <- function() {
  later::later(function() {
    alive <- tryCatch({
      result <- system2("pgrep", args=c("-f", "monitor_V2.py --inner"),
...
# Ctrl+C (SIGINT) 시 모니터 종료 후 R 서버 종료
shutdown <- function() {
  message()
  message(SEP)
  log_info("Ctrl+C detected — shutting down...")
  system("pkill -f 'monitor_V2.py'",
         ignore.stdout=TRUE, ignore.stderr=TRUE)
  log_ok("Monitor closed")
  message(SEP)
  quit(save="no", status=0)
...
tryCatch(
  ...
  error     = function(e) {
    log_err("Server error: ", conditionMessage(e))
    shutdown()
  }
)
